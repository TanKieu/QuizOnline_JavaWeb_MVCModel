    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if(request.getParameter("txtQuestion")!=null){
                questionName=request.getParameter("txtQuestion");
            }
            if(request.getParameter("txtSubject")!=null){
                SubjectID=request.getParameter("txtSubject");
            }
            ...
            searchResult= dao.searchQuestion(questionName, status, SubjectID);
            session.removeAttribute("QUESTION");
            session.setAttribute("QUESTION", searchResult);
        } catch (SQLException ex) {
            log("SearchServlet_SQLException "+ex.getMessage() );
        } catch (NamingException ex) {
            log("SearchServlet_NamingException "+ex.getMessage());
        }finally{
            String urlRewriting="adminPage?&txtQuestion="+questionName+"&txtSubject="+SubjectID+"&cbStatus=ON";
